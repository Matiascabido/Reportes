...
$sql = "  SELECT C.ID_SOLICITUD as [Credito], (select max(pag_fecha_pago) from fbc_pago where pag_id in (select id_pago from fbc_cuota where id_credito = c.cre_id)) as [Fecha Ult. Pago], P.PER_APELLIDO + ' ' + P.PER_NOMBRE  as [Apellido_y_Nombres], ";
$sql .= " P.PER_TIPO_DOC as Tipo, P.PER_NUM_DOC as [Numero de Documento], substring(P.per_sexo,1,1) as [Sexo],   ";
...
$sql .= " (select sum(cc.cuo_monto_capital+cc.cuo_monto_interes_financiero_1) from fbc_cuota cc where cc.id_credito = c.cre_id and cc.cuo_plan = 'NORMAL') as [Monto Credito],    ";
...
$sql .= " (select cuo_vencimiento_1 from fbc_cuota where cuo_id = (select max(cuo_id) from fbc_cuota where id_credito = c.cre_id)) as [FECHA ULT. PAGO],   ";
...
$sql .= " pr.pro_nombre as [Programa]    FROM FBC_CREDITO C    INNER JOIN FBC_SOLICITUD S ON S.SOL_ID = C.ID_SOLICITUD    INNER JOIN FBC_PERSONA P ON P.PER_ID = S.ID_TITULAR    INNER JOIN FBC_LOCALIDAD L ON L.LOC_ID = P.ID_LOCALIDAD    inner join fbc_programa pr on pr.pro_id = s.id_programa_solicitado    WHERE 1 = 1 And c.cre_fecha_efectivizacion <= EOMONTH(datefromparts(" . $anio . "," . $mes . ",1))    and (C.CRE_MONTO_OTORGADO - isnull(((SELECT SUM(CU.CUO_MONTO_CAPITAL) FROM FBC_CUOTA CU  WHERE CU.ID_CREDITO = C.CRE_ID AND CU.ID_PAGO IS not NULL)),0)) > 0";
$sql .= " and s.sol_id not in (select id_solicitud from FBC_SEGUIMIENTO_SOLICITUD where sso_estado in ('CIERRE_ANTICIPADO','CIERRE_EXTRAORDINARIO','CERRADO') and sso_id in (select max(sso_id) from fbc_seguimiento_solicitud where id_solicitud = S.sol_id))";
...
$campos = array();
$tipos = array();
$tam = array("7", "13", "40", "9", "27", "9", "40", "29", "11", "20", "15", "16", "13", "9", "17", "17", "19", "21", "24", "20", "17", "20", "18", "19", "17", "15", "14", "50");
$tamf = "10";
...
while ($row = sqlsrv_fetch_array($stmt)) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B' . $cel, $row['Fecha Ult. Pago']); //Credito Nuevo
    $auxNombre = $row['Apellido_y_Nombres'];
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $cel, $auxNombre); //Apellido y Nombres
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $cel, $row['Tipo']); //Tipo Doc
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $cel, $row['Numero de Documento']); //Num Doc
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $cel, $row['Sexo']); //Sexo
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $cel, $row['Domicilio']); //Domicilio
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H' . $cel, $row['LOCALIDAD']); //LOCALIDAD
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I' . $cel, $row['Provincia']); //Provincia
    if (($row['FECHA DEL PAGARE'] != null) || ($row['FECHA DEL PAGARE'] != 0)) {
        $datetime = $row['FECHA DEL PAGARE'];
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J' . $cel, PHPExcel_Shared_Date::PHPToExcel($datetime));
...
//          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.$cel, );   //FECHA DEL PAGARÉ
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('K' . $cel, $row['Monto Pagare']); //Monto Pagare
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('L' . $cel, $row['Monto Credito']); //Monto Credito
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('M' . $cel, $row['Valor Cuota']); //Valor Cuota
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('N' . $cel, $row['Capital']); //Capital
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O' . $cel, $row['Cuotas Pactadas']); //Cuotas Pactadas

    $datetime2 = $row['FECHA 1er PAGO'];
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('P' . $cel, PHPExcel_Shared_Date::PHPToExcel($datetime2));
...
//          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('O'.$cel, $row['FECHA 1er PAGO']);   //FECHA 1° PAGO
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q' . $cel, PHPExcel_Shared_Date::PHPToExcel($datetime3));
...
//          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('P'.$cel, $row['FECHA ULT. PAGO']);   //FECHA ULT. PAGO
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('R' . $cel, PHPExcel_Shared_Date::PHPToExcel($datetime4));
...
//          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Q'.$cel, $row['FECHA VTO.PAGARE']);   //FECHA VTO.PAGARE
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('S' . $cel, 'Mensual'); //Cuotas Pactadas
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('T' . $cel, $row['CUOTAS PAGADAS']); //Cuotas Pagadas
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('U' . $cel, $row['SALDO DEUDOR']); //Saldo Deudor
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('V' . $cel, $row['SALDO EN MORA']); //Saldo Mora
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('W' . $cel, $row['CUOTAS EN MORA']); //Cuotas Mora
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('X' . $cel, $row['CUOTA CAPITAL']); //Cuota Capital
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Y' . $cel, $row['Calculo Mora']); //Calculo Mora
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('Z' . $cel, $row['Total Pagado']); //Total Pagado
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AA' . $cel, $row['Saldo Deudor Calculado']); //Saldo Dedudor Calculado
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AB' . $cel, $row['Programa']); //Programa
    $cel++;
}
