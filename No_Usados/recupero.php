...
set_time_limit(1000);
        ...
        $conns = sqlsrv_connect($server,$concetinfo);
    ...
    $sql = "SELECT C.ID_SOLICITUD, S.ID_TITULAR, E.EMP_NOMBRE AS [NOMBRE_EMPRENDIMIENTO], CONCAT(PE.PER_NOMBRE,' ',PE.PER_APELLIDO) AS [RAZON_TITULAR], 
        PE.PER_NUM_DOC AS [DNI_TITULAR], PE.PER_CUIL_CUIT AS [CUIT_TITULAR], L.LOC_NOMBRE AS LOCALIDAD, D.DTO_NOMBRE AS DEPARTAMENTO,  
        e.emp_actividad_principal as [Actividad], e.emp_actividad_principal_desc as [Subactividad], e.emp_barrio as [Barrio], 
        ...
        C.AUD_USR_INS AS USUARIO_CREACION, cast(c.CRE_MONTO_OTORGADO as decimal(19,2)) as [Monto Credito],  C.CRE_CUOTAS_OTORGADAS AS [Cuotas Otorgadas],
         ...
         CAST(((SELECT COUNT(*) FROM FBC_CUOTA CC WHERE CC.ID_CREDITO = C.CRE_ID AND CC.ID_PAGO IS NULL AND CC.CUO_VENCIMIENTO_1 < GETDATE()) * (SELECT TOP 1 CCC.CUO_MONTO_CAPITAL FROM FBC_CUOTA CCC WHERE CCC.ID_CREDITO = C.CRE_ID AND CCC.ID_PAGO IS NULL AND CCC.CUO_VENCIMIENTO_1 < GETDATE())) AS DECIMAL(19,2)) AS [MOROSIDAD CAPITAL],
          CAST(((SELECT COUNT(*) FROM FBC_CUOTA CC WHERE CC.ID_CREDITO = C.CRE_ID AND CC.ID_PAGO IS NULL AND CC.CUO_VENCIMIENTO_1 < GETDATE()) * (SELECT TOP 1 CCC.CUO_MONTO_CAPITAL+CCC.CUO_MONTO_INTERES_FINANCIERO_1 FROM FBC_CUOTA CCC WHERE CCC.ID_CREDITO = C.CRE_ID AND CCC.ID_PAGO IS NULL AND CCC.CUO_VENCIMIENTO_1 < GETDATE())) AS DECIMAL(19,2)) AS [MOROSIDAD CAPITAL E INTERES] , 
         ...
         (SELECT COUNT(CCC.CUO_ID) FROM FBC_CUOTA CCC WHERE CCC.ID_CREDITO = C.CRE_ID AND CCC.CUO_VENCIMIENTO_1 < GETDATE() AND CCC.ID_PAGO IS NULL) AS [CUOTASEN MORA] ,(SELECT COUNT(CCC.CUO_ID) FROM FBC_CUOTA CCC WHERE CCC.ID_CREDITO = C.CRE_ID AND CCC.ID_PAGO IS NULL) AS [CUOTAS ADEUDADAS] ,
        ...
        INNER JOIN FBC_PROGRAMA_GASTOS PG ON PG.ID_PROGRAMA = PR.PRO_ID AND PG.PGA_GASTO = 'INTERES_ANUAL' ORDER BY S.SOL_ID";        
        $stmt=sqlsrv_query($conns,$sql);  
        ...
        if (($row['ULTIMO_PAGO'] != null) || ($row['ULTIMO_PAGO'] != 0))
          ...
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AG'.$cel, $row['CUOTAS ADEUDADAS']);   //Credito Nuevo4
        ...
        if (($row['PRIMERA_CUOTA_VENCIDA ORIGINAL'] != null) || ($row['PRIMERA_CUOTA_VENCIDA ORIGINAL'] != 0))
          {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AH'.$cel, PHPExcel_Shared_Date::PHPToExcel($datetime6));
        ...
        if (($row['PRIMERA_CUOTA_VENCIDA REFINANCIACION'] != null) || ($row['PRIMERA_CUOTA_VENCIDA REFINANCIACION'] != 0))
          {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AI'.$cel, PHPExcel_Shared_Date::PHPToExcel($datetime7));
        ...
        if (($row['ULTIMA_CUOTA_ABONADA ORIGINAL'] != null) || ($row['ULTIMA_CUOTA_ABONADA ORIGINAL'] != 0))
          {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AJ'.$cel, PHPExcel_Shared_Date::PHPToExcel($datetime5));
        ...
        if (($row['ULTIMA_CUOTA_ABONADA REFINANCIACION'] != null) || ($row['ULTIMA_CUOTA_ABONADA REFINANCIACION'] != 0))
          {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AK'.$cel, PHPExcel_Shared_Date::PHPToExcel($datetime8));
        ...
        if (($row['FECHA_ULTIMO_PAGO ORIGINAL'] != null) || ($row['FECHA_ULTIMO_PAGO ORIGINAL'] != 0))
          {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AL'.$cel, PHPExcel_Shared_Date::PHPToExcel($datetime9));
        ...
        if (($row['FECHA_ULTIMO_PAGO REFINANCIACION'] != null) || ($row['FECHA_ULTIMO_PAGO REFINANCIACION'] != 0))
          {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AM'.$cel, PHPExcel_Shared_Date::PHPToExcel($datetime10));
        ...
        if (($row['FECHA_ULTIMA_ACTUALIZACION'] != null) || ($row['FECHA_ULTIMA_ACTUALIZACION'] != 0))
          {
            ...
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AN'.$cel, PHPExcel_Shared_Date::PHPToExcel($datetime11));
            $objPHPExcel->getActiveSheet()->getStyle('AN'.$cel)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_DATE_DATETIME);
          }
          else
            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AN'.$cel, '');

          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AO'.$cel, $row['ULTIMA_NOVEDAD']);   //Credito Nuevo
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AP'.$cel, $row['INTERES ORIGINAL']);   //Credito Nuevo
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AQ'.$cel, $row['INTERES REFINANCIADO']);   //Credito Nuevo
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AR'.$cel, $row['DEUDA CAPITAL']);   //Credito Nuevo
          $objPHPExcel->setActiveSheetIndex(0)->setCellValue('AS'.$cel, $row['SOL_PER_MAIL']);   //Credito Nuevo
        ...
        header('Content-Disposition: attachment;filename="Recupero.xls"');
