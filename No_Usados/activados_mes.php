...
$sql = " SELECT C.ID_SOLICITUD,  CONCAT(S.SOL_PER_APELLIDO,' ', S.SOL_PER_NOMBRE) AS TITULAR, PE.PER_NUM_DOC  AS DNI, PE.PER_CUIL_CUIT AS CUIT,
             ...
             0 AS [CUOTAS EN MORA],  0 AS [CUOTAS ADEUDADAS], 0  AS MOROSIDAD, 0 AS  [DEUDA BASE], DO.DOM_TEXTO AS   ACTIVIDAD, e.emp_actividad_principal_desc as [ACTIVIDAD MENOR]
             ...
             INNER JOIN FBC_DOMINIO DO ON DO.DOM_CLAVE = E.EMP_ACTIVIDAD_PRINCIPAL
...
while ($row = sqlsrv_fetch_array($stmt)) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('T' . $cel, $row['ACTIVIDAD']); //Programa
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('U' . $cel, $row['ACTIVIDAD MENOR']); //Programa
    $cel++;
}
